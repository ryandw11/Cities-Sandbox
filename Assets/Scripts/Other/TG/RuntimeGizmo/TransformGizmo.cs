...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		public KeyCode SetScaleType = KeyCode.T;
		public KeyCode SetSpaceToggle = KeyCode.Y;
		public LayerMask buildingMask;
        public Dropdown insDrop;
		...
		/*
		 * Get Target to grab the transformer.
		 * Modified for better use.
		 */
	
		void GetTarget()
		{
			if(selectedAxis == Axis.None && Input.GetMouseButtonDown(insDrop.value)) //Changed this ------------------
			{
				Ray ray = myCamera.ScreenPointToRay (Input.mousePosition);
				RaycastHit hitInfo; 
				if(Physics.Raycast (ray.origin, ray.direction, out hitInfo, Mathf.Infinity, buildingMask))
				{
						target = hitInfo.transform.root.transform;
				}else{
					target = null;
				}
			}

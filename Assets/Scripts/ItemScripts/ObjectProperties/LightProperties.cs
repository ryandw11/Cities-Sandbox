...
public class LightProperties : MonoBehaviour, ObjectProperties
{
    ...
    public virtual void Display(GameObject content)
    {
        ...
        // Is Night only
        g.transform.GetChild(2).GetComponent<Toggle>().isOn = nightOnly;
        g.transform.GetChild(2).GetComponent<Toggle>().onValueChanged.AddListener(toggleValue =>
        {
            nightOnly = toggleValue;
            GetComponent<TimeControlLighting>().UpdateLightingCondition();
        });
        // Light Color
        g.transform.GetChild(3).GetComponent<ColorPicker>().CurrentColor = lightColor;
        g.transform.GetChild(3).GetComponent<ColorPicker>().onValueChanged.AddListener(newColor =>
        {
            lightColor = newColor;
            gameObject.GetComponentInChildren<Light>().color = lightColor;
        });
        // Object Color
        g.transform.GetChild(5).GetComponent<ColorPicker>().CurrentColor = objectColor;
        g.transform.GetChild(5).GetComponent<ColorPicker>().onValueChanged.AddListener(newColor =>
        {
            objectColor = newColor;
            gameObject.transform.GetChild(0).GetComponent<Renderer>().material.color = objectColor;
        });
        content.GetComponent<RectTransform>().sizeDelta = new Vector2(0, 470);
